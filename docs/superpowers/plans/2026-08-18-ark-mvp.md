...
- The reference corpus is a real community's minutes. It is never committed to this repo. Fixtures are either a handful of hand-picked HTML files or generated synthetic text; the real corpus is reached only through the `ARK_CORPUS_DIR` environment variable.
...
- Create: `ui/test/fixtures/gdocs/*.html` (3 files, copied from the the source HTML export)
- Modify: `ui/src/App.svelte`
...
So: **read** three files under `$ARK_CORPUS_DIR/minutes/html/` to learn their markup
...
Run: `nix develop -c npm run applet-dev`, then import `$ARK_CORPUS_DIR/minutes/by-meeting/ad-hoc/` (4 files).
