...
`ui/src/render.ts` — the only place markdown becomes HTML:
...
`ui/src/render.test.ts` — the regression test for the sanitization boundary:
...
**Fixture sourcing** (do this before Step 1). The markup is what is under test; the prose is a
real community's private minutes and must not enter this repo — not even redacted, because
redaction is a judgement call made once and wrong forever.

So: **read** three files under `~/code/the reference corpus/minutes/html/` to learn their markup
patterns, then **write** three fixtures from scratch into `ui/test/fixtures/gdocs/` that
reproduce those patterns exactly with entirely invented content. Pick the three to cover:

1. Google-Docs inline-CSS emphasis — the `<b style="font-weight:normal" id="docs-internal-guid-…">`
   wrapper with `<span style="font-weight:700">` and `<span style="font-style:italic">` inside.
2. A record containing a table.
3. One of the 16 plain-text-format records whose body contains literal `<p>` tags shown as text
   (node 1617, Buildings and Land 2019-03-31).

Copy the *structure* — tag nesting, attribute shapes, style strings, entity escaping, stray
whitespace — and invent the words. A fixture whose markup matches and whose sentences are about
a fictional committee tests the converter exactly as well, and carries nothing private.
...
// Keyed the way ArkIndex.keyOf keys them — base64, not the raw number. Plain
// '1'/'2'/'3' keys silently never match, and the folder filter then returns
...
In `App.svelte`: build a `SearchStore` after the document load, call `search.rebuild()`, and render `SearchBar` above the middle pane. Count `includeTrashed` as a filter when deciding whether to show `SearchResults` instead of the plain list — otherwise ticking "Include trashed" with nothing else set leaves `DocumentList` on screen, which hides trashed documents by its own rule, and the checkbox appears to do nothing. Derive `SearchBar`'s `authors` prop from the store — one entry per distinct `doc.author`, labelled with the first eight characters of the base64 key (a `<profile-detail>` element from `@holochain-open-dev/profiles` can replace the label later without touching the filter itself):
