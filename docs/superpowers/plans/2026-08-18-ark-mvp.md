...
export interface PlannedDoc {
  ...
  import_id: string;
  body: string;
  /** Basenames from the `attachments` front-matter key, if any. */
  attachments: string[];
}
...
- Modify: `ui/package.json` (add `test:perf` script). `.github/workflows/test.yaml` needs no change — `vitest.config.ts` already globs the new test file.
...
/**
 * Synthetic stand-in for the reference corpus: same document count, matching
 * total word count within a fraction of a percent, a long-tailed body-length
 * distribution and the same folder spread. Real minutes are a community's private records and are
...
export function generateCorpus(count: number, seed: number): DocumentSummary[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const words = Math.round(72 + Math.pow(random(), 3) * 1800);
