...
describe('amendments', () => {
  ...
  it('keeps a deeper offline branch visible in the history', async () => {
    await runScenario(async (scenario) => {
      ...
      // Bob forks off v1 while Alice builds a two-deep branch from the same point.
      // Whichever branch wins, NOTHING may disappear from the history: a greedy
      // walk would drop Alice's second edit entirely if Bob's edit won the fork.
      await Promise.all([
...
<script lang="ts">
  ...
  let { versions }: { versions: DocumentVersion[] } = $props();
    ...
    {#each versions as version, i (encodeHashToBase64(version.action))}
      ...
      <li>
        <button onclick={() => (expanded = expanded === id ? null : id)}>
          ...
          {#if i === versions.length - 1}<em>current</em>{/if}
        </button>
        {#if expanded === id}
