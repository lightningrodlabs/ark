...
| `stores/tree.ts` | tree store: fetch heads, merge, expose folders |
| `stores/documents.ts` | document store: chunked load, by-original map, signal patching |
| `stores/search.ts` | index lifecycle, wraps `search/` |
| `stores/signals.ts` | signal subscription, peer list from weave, reconcile timer |
...
Note: the `verify` job calls `npm run verify`, which needs the `ui` workspace. Until Task 9 creates it, add a temporary `"verify": "echo no ui yet"` override — replace it with the real script in Task 9, Step 5.
...
- Produces:
  ...
  - `latest_of(original: ActionHash) -> ExternResult<Option<Record>>` — walks the update chain and applies `pick_head` at every branch.
...
**Interfaces:**
- Consumes: `pick_head`, `latest_of` from Task 3; `ALL_TREES` from Task 2.
- Produces:
  ...
  - Externs: `get_folder_tree(()) -> Vec<TreeHead>`, `update_folder_tree(UpdateFolderTreeInput { folders: Vec<Folder> }) -> ActionHash`.
...
describe('folder tree', () => {
  ...
  it('exposes both heads when two agents edit concurrently', async () => {
    await runScenario(async (scenario) => {
      ...
      // Neither agent's folder is lost — the union across heads has both.
...
pub fn get_folder_tree(_: ()) -> ExternResult<Vec<TreeHead>> {
    ...
    for root in tree_roots()? {
        let Some(record) = latest_of(root)? else { continue };
...
pub fn update_folder_tree(input: UpdateFolderTreeInput) -> ExternResult<ActionHash> {
    ...
    let heads = get_folder_tree(())?;
    match heads
        .iter()
        .max_by(|a, b| a.timestamp.cmp(&b.timestamp).then(a.action.cmp(&b.action)))
    {
        // Update the newest head. Any older forked tip is left alone: every
        // folder it carries is superseded by a newer action for the same id in
        // this write, so the UI's union merge ignores it.
        Some(head) => update_entry(head.action.clone(), EntryTypes::FolderTree(tree)),
        None => {
...
Replace the temporary `verify` script in the root `package.json` with the real one:
...
**Files:**
- Create: `ui/src/tree/merge.ts`, `ui/src/tree/paths.ts`, `ui/src/stores/tree.ts`
...
**Files:**
- Create: `ui/src/stores/documents.ts`
...
```ts
import { describe, expect, it, vi } from 'vitest';
import { DocumentStore, key } from './documents';
import type { ArkClient } from '../ark-client';
...
Run: `npm run test -w ui`
Expected: FAIL — `ui/src/stores/documents.ts` does not exist.

- [ ] **Step 3: Write minimal implementation**

`ui/src/stores/documents.ts`:
...
<script lang="ts">
  ...
  import { key } from '../stores/documents';
...
**Files:**
- Create: `ui/src/search/{query.ts,snippet.ts,index.ts}`, `ui/src/stores/search.ts`
...
describe('ArkIndex', () => {
  ...
  it('excludes trashed documents unless asked', () => {
    const index = makeIndex();
    index.setTrashed(new Set(['AAAA']));
    // Trashed keys are base64 hashes; use the real key for doc 1.
...
**Files:**
- Create: `ui/src/stores/signals.ts`
- Modify: `ui/src/App.svelte`
...
```ts
import { describe, expect, it } from 'vitest';
import { needsReconcile, peersExcludingSelf } from './signals';
...
Run: `npm run test -w ui`
Expected: FAIL — `ui/src/stores/signals.ts` does not exist.

- [ ] **Step 3: Write minimal implementation**

`ui/src/stores/signals.ts`:
...
Import `WeaveClient` as a type in `signals.ts`:
