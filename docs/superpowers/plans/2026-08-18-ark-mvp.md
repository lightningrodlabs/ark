...
```json
{
  ...
  "scripts": {
    ...
    "start": "AGENTS=2 hc s clean && npm run build:happ && UI_PORT=8888 concurrently \"npm start -w ui\" \"npm run launch:happ\""
  },
  "devDependencies": {
...
describe('folder tree', () => {
  ...
  it('exposes every head when two agents started their own tree', async () => {
    await runScenario(async (scenario) => {
      const [alice, bob] = await scenario.addPlayersWithApps([appSource, appSource]);

      // Deliberately NOT shared yet. Each agent, unaware of the other, creates
      // its own tree root. This produces multiple heads deterministically —
      // unlike racing two updates against one root, where gossip usually
      // linearises the writes and no fork forms at all.
      await call(alice, 'update_folder_tree', { folders: [folder('a1', 'Alice folder')] });
      await call(bob, 'update_folder_tree', { folders: [folder('b1', 'Bob folder')] });

      await scenario.shareAllAgents();
      await dhtSync([alice, bob], arkCell(alice).cell_id[0]);

      const heads = await call<TreeHead[]>(alice, 'get_folder_tree', null);
      // Every head must come back. Collapsing to a single winner here would
      // silently drop one agent's folder, and the UI's union merge — which is
      // the only thing that reconciles them — would never see it.
  ...
  it('loses no folder when two agents edit the same tree concurrently', async () => {
    await runScenario(async (scenario) => {
      ...
      // the chain or gossip linearises them is outside the test's control — but
      // either way NO folder may be lost, which is what the union merge and the
      // `deleted` tombstone exist to guarantee.
      const heads = await call<TreeHead[]>(alice, 'get_folder_tree', null);
      const ids = new Set(heads.flatMap((h) => h.folders.map((f) => f.id)));
      expect(ids.has('a1')).toBe(true);
      expect(ids.has('b1')).toBe(true);
...
Run: `npm run test -w ui`
Expected: PASS — nine `mergeHeads`/`liveFolders` cases and six `paths` cases.
