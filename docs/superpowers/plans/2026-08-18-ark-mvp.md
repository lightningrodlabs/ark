...
**Files:**
- Create: `dnas/ark/zomes/coordinator/ark/src/resolve.rs`
- Modify: `dnas/ark/zomes/coordinator/ark/src/{document.rs,types.rs,lib.rs}`
- Test: `dnas/ark/zomes/coordinator/ark/src/resolve.rs` (inline `#[cfg(test)]`), `tests/src/ark/amend.test.ts`
...
Rust unit test, appended to `dnas/ark/zomes/coordinator/ark/src/resolve.rs`:
...
`dnas/ark/zomes/coordinator/ark/src/resolve.rs` (above the test module):
...
If the host-target build of the crate fails because an hdk dependency will not compile off-wasm, move `pick_head` and its tests into a dependency-free module `dnas/ark/zomes/coordinator/ark/src/pick.rs` that has no `use hdk::` at all, and gate the rest of the crate's test compilation with `#![cfg(target_arch = "wasm32")]` on the hdk-using modules. The function body and tests are unchanged.
